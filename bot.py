# bot.py — Portal SimonSports — Publicador Automático (X, Facebook, Telegram, Discord, Pinterest)
# Rev: 2025-11-21r — REVERT SAFE
# - NÃO filtra por data; publica quando a coluna da rede está VAZIA (limpeza de invisíveis)
# - X: posta a IMAGEM; se X_REPLY_WITH_LINK_BELOW=true => responde com link(E) + canais (P e Q)
# - Mantém comportamento de postar nas 2 contas do X o MESMO conteúdo (se configuradas)
# - NÃO olha coluna “Enfileirado”
# - Texto mínimo nas demais redes: callout + links (legenda única)

import os, re, io, glob, json, time, base64
import pytz, tweepy, requests, datetime as dt
from threading import Thread
from collections import defaultdict
from dotenv import load_dotenv

# Google Sheets
import gspread
from oauth2client.service_account import ServiceAccountCredentials

# Imagem oficial aprovada
from app.imaging import gerar_imagem_loteria

load_dotenv()
TZ = pytz.timezone("America/Sao_Paulo")

SHEET_ID  = os.getenv("GOOGLE_SHEET_ID", "").strip()
SHEET_TAB = os.getenv("SHEET_TAB", "ImportadosBlogger2").strip()

TARGET_NETWORKS = [s.strip().upper() for s in os.getenv("TARGET_NETWORKS", "X").split(",") if s.strip()]
DRY_RUN = os.getenv("DRY_RUN", "false").strip().lower() == "true"

GLOBAL_TEXT_MODE     = (os.getenv("GLOBAL_TEXT_MODE", "") or "").strip().upper()
X_TEXT_MODE          = (os.getenv("X_TEXT_MODE", "") or "").strip().upper()
FACEBOOK_TEXT_MODE   = (os.getenv("FACEBOOK_TEXT_MODE", "") or "").strip().upper()
TELEGRAM_TEXT_MODE   = (os.getenv("TELEGRAM_TEXT_MODE", "") or "").strip().upper()
DISCORD_TEXT_MODE    = (os.getenv("DISCORD_TEXT_MODE", "") or "").strip().upper()
PINTEREST_TEXT_MODE  = (os.getenv("PINTEREST_TEXT_MODE", "") or "").strip().upper()

VALID_TEXT_MODES = {"IMAGE_ONLY", "TEXT_AND_IMAGE", "TEXT_ONLY"}
def get_text_mode(rede: str) -> str:
    specific = {
        "X": X_TEXT_MODE, "FACEBOOK": FACEBOOK_TEXT_MODE, "TELEGRAM": TELEGRAM_TEXT_MODE,
        "DISCORD": DISCORD_TEXT_MODE, "PINTEREST": PINTEREST_TEXT_MODE,
    }.get(rede, "")
    mode = (specific or GLOBAL_TEXT_MODE or "TEXT_AND_IMAGE").upper()
    return mode if mode in VALID_TEXT_MODES else "TEXT_AND_IMAGE"

# Colunas (base 1)
COL_LOTERIA, COL_CONCURSO, COL_DATA, COL_NUMEROS, COL_URL = 1, 2, 3, 4, 5
COL_URL_IMAGEM, COL_IMAGEM = 6, 7
COL_TG1, COL_TG2 = 16, 17  # P e Q

COL_STATUS_X         = int(os.getenv("COL_STATUS_X", "8"))
COL_STATUS_TELEGRAM  = int(os.getenv("COL_STATUS_TELEGRAM", "10"))
COL_STATUS_DISCORD   = int(os.getenv("COL_STATUS_DISCORD", "13"))
COL_STATUS_PINTEREST = int(os.getenv("COL_STATUS_PINTEREST", "14"))
COL_STATUS_FACEBOOK  = int(os.getenv("COL_STATUS_FACEBOOK", "15"))

COL_STATUS_REDES = {
    "X": COL_STATUS_X, "TELEGRAM": COL_STATUS_TELEGRAM, "DISCORD": COL_STATUS_DISCORD,
    "PINTEREST": COL_STATUS_PINTEREST, "FACEBOOK": COL_STATUS_FACEBOOK
}

X_POST_IN_ALL_ACCOUNTS     = os.getenv("X_POST_IN_ALL_ACCOUNTS", "true").strip().lower() == "true"
POST_X_WITH_IMAGE          = os.getenv("POST_X_WITH_IMAGE", "true").strip().lower() == "true"
X_REPLY_WITH_LINK_BELOW    = os.getenv("X_REPLY_WITH_LINK_BELOW", "false").strip().lower() == "true"
LINKS_TELEGRAM_ENV         = [s.strip() for s in os.getenv("LINKS_TELEGRAM", "").split(",") if s.strip()]

POST_FB_WITH_IMAGE         = os.getenv("POST_FB_WITH_IMAGE", "true").strip().lower() == "true"
POST_TG_WITH_IMAGE         = os.getenv("POST_TG_WITH_IMAGE", "true").strip().lower() == "true"
POST_PINTEREST_WITH_IMAGE  = os.getenv("POST_PINTEREST_WITH_IMAGE", "true").strip().lower() == "true"

USE_KIT_IMAGE_FIRST = os.getenv("USE_KIT_IMAGE_FIRST", "false").strip().lower() == "true"
KIT_OUTPUT_DIR      = os.getenv("KIT_OUTPUT_DIR", "output").strip()

ENABLE_KEEPALIVE    = os.getenv("ENABLE_KEEPALIVE", "false").strip().lower() == "true"
KEEPALIVE_PORT      = int(os.getenv("KEEPALIVE_PORT", "8080"))

MAX_PUBLICACOES_RODADA = int(os.getenv("MAX_PUBLICACOES_RODADA", "30"))
PAUSA_ENTRE_POSTS      = float(os.getenv("PAUSA_ENTRE_POSTS", "2.0"))

def _log(*a): print(f"[{dt.datetime.now(TZ).strftime('%Y-%m-%d %H:%M:%S')}]", *a, flush=True)
def _safe_len(row, idx): return len(row) >= idx

def _clean_invis(v):
    s = ("" if v is None else str(v))
    s = s.strip()
    for ch in ["\u200B","\u200C","\u200D","\uFEFF","\u2060"]: s = s.replace(ch,"")
    return s

def _is_empty_status(v): return _clean_invis(v) == ""

def _gs_client():
    sa_json = os.getenv("GOOGLE_SERVICE_JSON", "").strip()
    scopes = ["https://www.googleapis.com/auth/spreadsheets","https://www.googleapis.com/auth/drive"]
    if sa_json:
        info = json.loads(sa_json)
        creds = ServiceAccountCredentials.from_json_keyfile_dict(info, scopes)
    else:
        path = "service_account.json"
        if not os.path.exists(path): raise RuntimeError("Credencial Google ausente (GOOGLE_SERVICE_JSON / service_account.json).")
        creds = ServiceAccountCredentials.from_json_keyfile_name(path, scopes)
    return gspread.authorize(creds)

def _open_ws():
    if not SHEET_ID: raise RuntimeError("GOOGLE_SHEET_ID não definido.")
    return _gs_client().open_by_key(SHEET_ID).worksheet(SHEET_TAB)

def marcar_publicado(ws, rownum, rede, value=None):
    col = COL_STATUS_REDES.get(rede)
    if not col: return
    value = value or f"Publicado {rede} em {dt.datetime.now(TZ).strftime('%d/%m/%Y %H:%M')}"
    ws.update_cell(rownum, col, value)

def coleta_candidatos_para(ws, rede: str):
    linhas = ws.get_all_values()
    if len(linhas) <= 1: return []
    data = linhas[1:]
    cand = []
    col_status = COL_STATUS_REDES.get(rede)
    for rindex, row in enumerate(data, start=2):
        status_val = row[col_status - 1] if len(row) >= col_status else ""
        if _is_empty_status(status_val): cand.append((rindex, row))
    _log(f"[{rede}] Candidatas: {len(cand)}")
    return cand

# ========= Imagem =========

def _guess_slug(s: str) -> str:
    s = (s or "").lower()
    s = re.sub(r"[áàâãä]", "a", s); s = re.sub(r"[éèêë]", "e", s); s = re.sub(r"[íìîï]", "i", s)
    s = re.sub(r"[óòôõö]", "o", s); s = re.sub(r"[úùûü]", "u", s); s = s.replace("ç","c")
    s = re.sub(r"[^a-z0-9- ]+","", s); s = re.sub(r"\s+","-", s).strip("-"); return s

def _try_load_kit_image(row):
    if not USE_KIT_IMAGE_FIRST: return None
    try:
        loteria  = row[COL_LOTERIA - 1] if _safe_len(row, COL_LOTERIA) else ""
        concurso = row[COL_CONCURSO - 1] if _safe_len(row, COL_CONCURSO) else ""
        data_br  = row[COL_DATA - 1]     if _safe_len(row, COL_DATA)     else ""
        slug = _guess_slug(loteria)
        patterns = []
        if concurso: patterns += [os.path.join(KIT_OUTPUT_DIR, f"*{slug}*{_guess_slug(concurso)}*.jp*g")]
        if data_br:  patterns += [os.path.join(KIT_OUTPUT_DIR, f"*{slug}*{_guess_slug(data_br)}*.jp*g")]
        patterns += [os.path.join(KIT_OUTPUT_DIR, f"{slug}*.jp*g")]
        for pat in patterns:
            files = sorted(glob.glob(pat))
            if files:
                with open(files[0],"rb") as f:
                    b = io.BytesIO(f.read()); b.seek(0); return b
        return None
    except Exception as e:
        _log(f"[KIT] erro: {e}")
        return None

def _build_image_from_row(row):
    b = _try_load_kit_image(row)
    if b: return b
    loteria  = row[COL_LOTERIA - 1] if _safe_len(row, COL_LOTERIA) else "Loteria"
    concurso = row[COL_CONCURSO - 1] if _safe_len(row, COL_CONCURSO) else ""
    data_br  = row[COL_DATA - 1]     if _safe_len(row, COL_DATA)     else ""
    numeros  = row[COL_NUMEROS - 1]  if _safe_len(row, COL_NUMEROS)  else ""
    url_res  = row[COL_URL - 1]      if _safe_len(row, COL_URL)      else ""
    return gerar_imagem_loteria(str(loteria), str(concurso), str(data_br), str(numeros), str(url_res))

# ========= Texto (legenda/caption) =========

def _mk_caption(row):
    url = _clean_invis(row[COL_URL - 1] if _safe_len(row, COL_URL) else "")
    tg1 = _clean_invis(row[COL_TG1 - 1] if _safe_len(row, COL_TG1) else "")
    tg2 = _clean_invis(row[COL_TG2 - 1] if _safe_len(row, COL_TG2) else "")
    if not tg1 and not tg2 and LINKS_TELEGRAM_ENV:
        if len(LINKS_TELEGRAM_ENV) >= 1: tg1 = LINKS_TELEGRAM_ENV[0]
        if len(LINKS_TELEGRAM_ENV) >= 2: tg2 = LINKS_TELEGRAM_ENV[1]
    linhas = []
    if url: linhas.append("Resultado completo aqui >>>")
    if url: linhas.append(url)
    if tg1: linhas.append(tg1)
    if tg2: linhas.append(tg2)
    return "\n".join(linhas).strip()

# ========= Publicadores =========
# X

TW1 = {
    "api_key": os.getenv("TWITTER_API_KEY_1",""),
    "api_secret": os.getenv("TWITTER_API_SECRET_1",""),
    "access_token": os.getenv("TWITTER_ACCESS_TOKEN_1",""),
    "access_secret": os.getenv("TWITTER_ACCESS_SECRET_1",""),
}
TW2 = {
    "api_key": os.getenv("TWITTER_API_KEY_2",""),
    "api_secret": os.getenv("TWITTER_API_SECRET_2",""),
    "access_token": os.getenv("TWITTER_ACCESS_TOKEN_2",""),
    "access_secret": os.getenv("TWITTER_ACCESS_SECRET_2",""),
}

class XAccount:
    def __init__(self, label, api_key, api_secret, access_token, access_secret):
        self.label = label
        self.client_v2 = tweepy.Client(consumer_key=api_key, consumer_secret=api_secret,
                                       access_token=access_token, access_token_secret=access_secret)
        auth = tweepy.OAuth1UserHandler(api_key, api_secret, access_token, access_secret)
        self.api_v1 = tweepy.API(auth)
        try:
            me = self.client_v2.get_me(); self.user_id = me.data.id if me and me.data else None
            self.handle = "@"+(me.data.username if me and me.data else label)
        except Exception:
            self.user_id = None; self.handle = f"@{label}"

def build_x_accounts():
    out = []
    def ok(d): return all(d.get(k) for k in ("api_key","api_secret","access_token","access_secret"))
    if ok(TW1): out.append(XAccount("ACC1", **TW1))
    if ok(TW2): out.append(XAccount("ACC2", **TW2))
    if not out: raise RuntimeError("X: nenhuma conta configurada.")
    return out

def x_upload_media(acc: XAccount, row):
    try:
        buf = _build_image_from_row(row)
        media = acc.api_v1.media_upload(filename="resultado.png", file=buf)
        return [media.media_id_string]
    except Exception as e:
        _log(f"[{acc.handle}] imagem: {e}"); return None

def publicar_em_x(ws, candidatos):
    contas = build_x_accounts()
    publicados = 0
    limite = min(MAX_PUBLICACOES_RODADA, len(candidatos))
    mode = get_text_mode("X")
    for rownum, row in candidatos[:limite]:
        caption = _mk_caption(row)
        ok_any = False
        if X_POST_IN_ALL_ACCOUNTS:
            for acc in contas:
                try:
                    if DRY_RUN:
                        _log(f"[X]{acc.handle} DRY_RUN — enviaria imagem; reply abaixo: {bool(X_REPLY_WITH_LINK_BELOW)}")
                        ok = True
                    else:
                        media_ids = x_upload_media(acc, row) if POST_X_WITH_IMAGE else None
                        resp = acc.client_v2.create_tweet(
                            text=(caption if mode=="TEXT_AND_IMAGE" else None) if not POST_X_WITH_IMAGE else (None if mode=="IMAGE_ONLY" else caption or None),
                            media_ids=media_ids if POST_X_WITH_IMAGE else None,
                        )
                        _log(f"[X]{acc.handle} OK → {resp.data['id']}")
                        if mode=="IMAGE_ONLY" and X_REPLY_WITH_LINK_BELOW and caption:
                            acc.client_v2.create_tweet(text=caption, in_reply_to_tweet_id=resp.data["id"])
                            _log(f"[X]{acc.handle} reply ✔")
                        ok = True
                except Exception as e:
                    _log(f"[X]{acc.handle} erro: {e}"); ok = False
                ok_any = ok_any or ok
                time.sleep(0.7)
        else:
            acc = contas[0]
            try:
                if DRY_RUN:
                    _log(f"[X]{acc.handle} DRY_RUN — enviaria imagem; reply: {bool(X_REPLY_WITH_LINK_BELOW)}")
                    ok_any = True
                else:
                    media_ids = x_upload_media(acc, row) if POST_X_WITH_IMAGE else None
                    resp = acc.client_v2.create_tweet(
                        text=(caption if mode=="TEXT_AND_IMAGE" else None) if not POST_X_WITH_IMAGE else (None if mode=="IMAGE_ONLY" else caption or None),
                        media_ids=media_ids if POST_X_WITH_IMAGE else None,
                    )
                    _log(f"[X]{acc.handle} OK → {resp.data['id']}")
                    if mode=="IMAGE_ONLY" and X_REPLY_WITH_LINK_BELOW and caption:
                        acc.client_v2.create_tweet(text=caption, in_reply_to_tweet_id=resp.data["id"])
                        _log(f"[X]{acc.handle} reply ✔")
                    ok_any = True
            except Exception as e:
                _log(f"[X]{acc.handle} erro: {e}"); ok_any = False

        if ok_any and not DRY_RUN:
            marcar_publicado(ws, rownum, "X"); publicados += 1
        time.sleep(PAUSA_ENTRE_POSTS)
    _log(f"[X] Publicados: {publicados}")
    return publicados

# Facebook
FB_PAGE_IDS    = [s.strip() for s in os.getenv("FB_PAGE_IDS", "").split(",") if s.strip()]
FB_PAGE_TOKENS = [s.strip() for s in os.getenv("FB_PAGE_TOKENS", "").split(",") if s.strip()]

def _fb_post_text(page_id, page_token, message: str, link: str | None = None):
    url = f"https://graph.facebook.com/v19.0/{page_id}/feed"
    data={"message":message,"access_token":page_token}; 
    if link: data["link"]=link
    r=requests.post(url,data=data,timeout=25); r.raise_for_status(); return r.json().get("id")

def _fb_post_photo(page_id, page_token, caption: str, image_bytes: bytes):
    url = f"https://graph.facebook.com/v19.0/{page_id}/photos"
    files={"source":("resultado.png", image_bytes, "image/png")}
    data={"caption":caption,"published":"true","access_token":page_token}
    r=requests.post(url,data=data,files=files,timeout=40); r.raise_for_status(); return r.json().get("id")

def publicar_em_facebook(ws, candidatos):
    if not FB_PAGE_IDS or len(FB_PAGE_IDS)!=len(FB_PAGE_TOKENS):
        raise RuntimeError("Facebook: configure FB_PAGE_IDS e FB_PAGE_TOKENS (mesma ordem).")
    publicados=0; limite=min(MAX_PUBLICACOES_RODADA,len(candidatos)); mode=get_text_mode("FACEBOOK")
    for rownum,row in candidatos[:limite]:
        caption=_mk_caption(row) if mode!="TEXT_ONLY" else ""
        try:
            if DRY_RUN: _log("[Facebook] DRY_RUN"); ok=True
            else:
                if POST_FB_WITH_IMAGE:
                    buf=_build_image_from_row(row); img=buf.getvalue()
                    for pid,pt in zip(FB_PAGE_IDS,FB_PAGE_TOKENS):
                        _fb_post_photo(pid,pt,caption,img); _log(f"[Facebook]{pid} OK")
                else:
                    url = _clean_invis(row[COL_URL-1] if _safe_len(row,COL_URL) else "")
                    for pid,pt in zip(FB_PAGE_IDS,FB_PAGE_TOKENS):
                        _fb_post_text(pid,pt,caption, url or None); _log(f"[Facebook]{pid} OK")
                ok=True
        except Exception as e: _log(f"[Facebook] erro: {e}"); ok=False
        if ok and not DRY_RUN: marcar_publicado(ws,rownum,"FACEBOOK"); publicados+=1
        time.sleep(PAUSA_ENTRE_POSTS)
    _log(f"[Facebook] Publicados: {publicados}"); return publicados

# Telegram
TG_BOT_TOKEN = os.getenv("TG_BOT_TOKEN","").strip()
TG_CHAT_IDS  = [s.strip() for s in os.getenv("TG_CHAT_IDS","").split(",") if s.strip()]

def _tg_send_photo(token, chat_id, caption, image_bytes):
    url=f"https://api.telegram.org/bot{token}/sendPhoto"
    files={"photo":("resultado.png",image_bytes,"image/png")}
    data={"chat_id":chat_id,"caption":caption}
    r=requests.post(url,data=data,files=files,timeout=40); r.raise_for_status()
    return r.json().get("result",{}).get("message_id")

def publicar_em_telegram(ws, candidatos):
    if not TG_BOT_TOKEN or not TG_CHAT_IDS: raise RuntimeError("Telegram: configure TG_BOT_TOKEN e TG_CHAT_IDS.")
    publicados=0; limite=min(MAX_PUBLICACOES_RODADA,len(candidatos)); mode=get_text_mode("TELEGRAM")
    for rownum,row in candidatos[:limite]:
        caption=_mk_caption(row) if mode!="TEXT_ONLY" else ""
        try:
            if DRY_RUN: _log("[Telegram] DRY_RUN"); ok=True
            else:
                buf=_build_image_from_row(row); img=buf.getvalue()
                for cid in TG_CHAT_IDS:
                    _tg_send_photo(TG_BOT_TOKEN,cid,caption,img); _log(f"[Telegram]{cid} OK")
                ok=True
        except Exception as e: _log(f"[Telegram] erro: {e}"); ok=False
        if ok and not DRY_RUN: marcar_publicado(ws,rownum,"TELEGRAM"); publicados+=1
        time.sleep(PAUSA_ENTRE_POSTS)
    _log(f"[Telegram] Publicados: {publicados}"); return publicados

# Discord
DISCORD_WEBHOOKS=[s.strip() for s in os.getenv("DISCORD_WEBHOOKS","").split(",") if s.strip()]

def _discord_send(wh, content=None, image_bytes=None):
    data={"content":content or ""}; files=None
    if image_bytes: files={"file":("resultado.png",image_bytes,"image/png")}
    r=requests.post(wh,data=data,files=files,timeout=30); r.raise_for_status(); return True

def publicar_em_discord(ws, candidatos):
    if not DISCORD_WEBHOOKS: raise RuntimeError("Discord: defina DISCORD_WEBHOOKS.")
    publicados=0; limite=min(MAX_PUBLICACOES_RODADA,len(candidatos)); mode=get_text_mode("DISCORD")
    for rownum,row in candidatos[:limite]:
        caption=_mk_caption(row) if mode!="TEXT_ONLY" else ""
        try:
            if DRY_RUN: _log("[Discord] DRY_RUN"); ok=True
            else:
                buf=_build_image_from_row(row); img=buf.getvalue()
                for wh in DISCORD_WEBHOOKS: _discord_send(wh, caption or None, img); _log(f"[Discord] OK")
                ok=True
        except Exception as e: _log(f"[Discord] erro: {e}"); ok=False
        if ok and not DRY_RUN: marcar_publicado(ws,rownum,"DISCORD"); publicados+=1
        time.sleep(PAUSA_ENTRE_POSTS)
    _log(f"[Discord] Publicados: {publicados}"); return publicados

# Pinterest
PINTEREST_ACCESS_TOKEN=os.getenv("PINTEREST_ACCESS_TOKEN","").strip()
PINTEREST_BOARD_ID=os.getenv("PINTEREST_BOARD_ID","").strip()

def _pinterest_create_pin(token, board_id, title, description, link, image_bytes):
    url="https://api.pinterest.com/v5/pins"; headers={"Authorization":f"Bearer {token}"}
    payload={"board_id":board_id,"title":title[:100],"description":(description or "")[:500]}
    if link: payload["link"]=link
    payload["media_source"]={"source_type":"image_base64","content_type":"image/png","data":base64.b64encode(image_bytes).decode("utf-8")}
    r=requests.post(url,headers=headers,json=payload,timeout=40); r.raise_for_status(); return r.json().get("id")

def publicar_em_pinterest(ws, candidatos):
    if not (PINTEREST_ACCESS_TOKEN and PINTEREST_BOARD_ID): raise RuntimeError("Pinterest: configure token e board.")
    publicados=0; limite=min(MAX_PUBLICACOES_RODADA,len(candidatos)); mode=get_text_mode("PINTEREST")
    for rownum,row in candidatos[:limite]:
        loteria=row[COL_LOTERIA-1] if _safe_len(row,COL_LOTERIA) else "Loteria"
        concurso=row[COL_CONCURSO-1] if _safe_len(row,COL_CONCURSO) else ""
        title=f"{loteria} — Concurso {concurso}".strip(" —")
        caption=_mk_caption(row) if mode!="TEXT_ONLY" else ""
        try:
            if DRY_RUN: _log("[Pinterest] DRY_RUN"); ok=True
            else:
                buf=_build_image_from_row(row); img=buf.getvalue()
                _pinterest_create_pin(PINTEREST_ACCESS_TOKEN,PINTEREST_BOARD_ID,title,caption,
                                      _clean_invis(row[COL_URL-1] if _safe_len(row,COL_URL) else ""), img)
                _log("[Pinterest] OK"); ok=True
        except Exception as e: _log(f"[Pinterest] erro: {e}"); ok=False
        if ok and not DRY_RUN: marcar_publicado(ws,rownum,"PINTEREST"); publicados+=1
        time.sleep(PAUSA_ENTRE_POSTS)
    _log(f"[Pinterest] Publicados: {publicados}"); return publicados

# Keepalive opcional
def iniciar_keepalive():
    try: from flask import Flask
    except ImportError: _log("Flask não instalado; keepalive off."); return None
    app = Flask(__name__)
    @app.route("/") ; @app.route("/ping")
    def _r(): return "ok",200
    def run(): app.run(host="0.0.0.0", port=int(os.getenv("PORT", KEEPALIVE_PORT)), debug=False, use_reloader=False)
    th = Thread(target=run, daemon=True); th.start(); return th

def main():
    if ENABLE_KEEPALIVE: iniciar_keepalive()
    ws = _open_ws()
    for rede in TARGET_NETWORKS:
        if rede not in COL_STATUS_REDES: _log(f"[{rede}] não suportada."); continue
        cand = coleta_candidatos_para(ws, rede)
        if not cand: _log(f"[{rede}] nada a publicar."); continue
        if rede=="X": publicar_em_x(ws, cand)
        elif rede=="FACEBOOK": publicar_em_facebook(ws, cand)
        elif rede=="TELEGRAM": publicar_em_telegram(ws, cand)
        elif rede=="DISCORD": publicar_em_discord(ws, cand)
        elif rede=="PINTEREST": publicar_em_pinterest(ws, cand)
    _log("Concluído.")

if __name__=="__main__": main()