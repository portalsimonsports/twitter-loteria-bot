...
def parse_numeros(loteria_nome: str, numeros_str: str):
    ...
    if "lotofacil" in nome:
        ...
        if n <= 8: rows = [nums]
            ...
            mid = math.ceil(n/2); rows = [nums[:mid], nums[mid:]]
        else:
            terc = math.ceil(n/3); rows = [nums[:terc], nums[terc:2*terc], nums[2*terc:]]
...
def desenhar_loteca(draw: ImageDraw.ImageDraw, loteria_nome: str, numeros_str: str, area_box):
    ...
    for i in range(14):
        ...
        draw.text((_center(xs[3], xs[4]), (top+bot)/2), "×", font=f_team,
                  fill=(255,255,255) if res=="X" else (200,200,215), anchor="mm")
