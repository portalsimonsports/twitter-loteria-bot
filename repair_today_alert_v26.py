...
def _uploads_playlist(token: str) -> str:
    r=requests.get(f"{API}/channels",headers={"Authorization":f"Bearer {token}"},params={"part":"contentDetails","mine":"true"},timeout=30)
...
def _today_alert_id(token: str, date: str) -> str:
    ...
    r=requests.get(f"{API}/playlistItems",headers={"Authorization":f"Bearer {token}"},params={"part":"snippet,contentDetails","playlistId":playlist,"maxResults":25},timeout=30)
    r.raise_for_status()
...
def main() -> int:
    ...
    date=_today(cfg.timezone); targets=cal.targets_for_date(values,date)
    prize=cal.prize_highlight_for_date(values,date,targets)
    ...
    for account in listar_contas_youtube(cofre_cache):
        ...
        token=get_access_token(cid,sec,ref); vid=_today_alert_id(token,date)
        if not vid: continue
        upload_thumbnail(token,vid,thumb); updated+=1
