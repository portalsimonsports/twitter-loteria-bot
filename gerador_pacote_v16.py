...
from voice_narration_v16 import (
    extract_numbers,
    reveal_times_full,
    reveal_times_short,
    select_voice,
...
def _create_short(base_video: Path, data: Dict[str, Any], output_dir: Path, voice: str) -> Path:
    ...
    output = output_dir / f"short_{lottery}_{contest}_30s_voz_v16.mp4"
    ...
    with tempfile.TemporaryDirectory(prefix="portalsimonsports-short-v16-") as temp_dir:
        temp = Path(temp_dir)
...
def _create_full(base_video: Path, data: Dict[str, Any], output_dir: Path, voice: str) -> Path:
    ...
    lottery = v13.visual._slug(lottery_text) or "loteria"
    contest = v13.visual._slug(contest_text or "resultado") or "resultado"
    output = output_dir / f"video_completo_{lottery}_{contest}_{round(FULL_DURATION)}s_voz_v16.mp4"
    ...
    with tempfile.TemporaryDirectory(prefix="portalsimonsports-completo-v16-") as temp_dir:
        temp = Path(temp_dir)
...
def gerar_pacote(data: Dict[str, Any]) -> Dict[str, str]:
    ...
    print(
        f"[VÍDEO {VERSION}] Voz única: {presenter} | fala sincronizada com a dezena visível | "
